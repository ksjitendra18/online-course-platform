...
const BasicInformation = ({
  ...
  const handleCreateCourse = async (e: any) => {
    ...
    try {
      const parseResult = BasicInfoSchema.safeParse({
        courseName,
        courseSlug,
        courseDescription,
        isFree: courseFree,
      ...
      let reqBody = JSON.stringify(parseResult.data);

      if (reqMethod === "PATCH") {
        reqBody = JSON.stringify({ ...parseResult.data, courseId });
    ...
    <>
      <div className="auth-options w-full px-6 flex flex-col items-center justify-center">
        ...
        >
          ...
          >
            You can have some chapters for demo to be set as free.
              ...
              {isLoading ? (
                <>
                  <div className="flex items-center justify-center gap-2">
                    ...
                    {update ? "Updating Course..." : "Creating Course..."}
                  </div>
                </>
              ) : (
